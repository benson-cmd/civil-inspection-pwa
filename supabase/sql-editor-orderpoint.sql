-- Civil Inspection PWA setup for an existing Supabase project.
-- Paste this whole file into Supabase SQL Editor for orderpoint-dimension.
-- It stops if common table names already exist and were not created by this app.
...
begin
  ...
  where n.nspname = 'public'
    ...
    and coalesce(obj_description(c.oid, 'pg_class'), '') <> 'civil-inspection-pwa';

  if collisions is not null then
    raise exception
      'Stop: existing table name(s) found: %. Do not continue until these names are checked.',
      collisions;
  end if;
end $$;
...
begin
  ...
  if not exists (select 1 from pg_type where typnamespace = 'public'::regnamespace and typname = 'attachment_mode') then
    create type public.attachment_mode as enum ('upload', 'editor');
  end if;

  if not exists (select 1 from pg_type where typnamespace = 'public'::regnamespace and typname = 'attachment_status') then
...
comment on table public.profiles is 'civil-inspection-pwa';
comment on table public.projects is 'civil-inspection-pwa';
comment on table public.project_members is 'civil-inspection-pwa';
comment on table public.report_sections is 'civil-inspection-pwa';
comment on table public.attachments is 'civil-inspection-pwa';
comment on table public.targets is 'civil-inspection-pwa';
comment on table public.floors is 'civil-inspection-pwa';
comment on table public.inspection_points is 'civil-inspection-pwa';
comment on table public.photos is 'civil-inspection-pwa';
comment on table public.site_photos is 'civil-inspection-pwa';

create index if not exists projects_case_no_idx on public.projects(case_no);
create index if not exists project_members_user_id_idx on public.project_members(user_id);
create index if not exists report_sections_project_id_idx on public.report_sections(project_id);
create index if not exists attachments_project_id_idx on public.attachments(project_id);
create index if not exists targets_project_id_idx on public.targets(project_id);
create index if not exists floors_target_id_idx on public.floors(target_id);
create index if not exists inspection_points_floor_id_idx on public.inspection_points(floor_id);
create index if not exists photos_point_id_idx on public.photos(point_id);
create index if not exists site_photos_project_id_idx on public.site_photos(project_id);
...
drop policy if exists "report_sections_project_members" on public.report_sections;
...
drop policy if exists "attachments_project_members" on public.attachments;
...
drop policy if exists "targets_project_members" on public.targets;
...
drop policy if exists "floors_project_members" on public.floors;
...
drop policy if exists "inspection_points_project_members" on public.inspection_points;
...
drop policy if exists "photos_project_members" on public.photos;
...
drop policy if exists "site_photos_project_members" on public.site_photos;
...
-- Run this after your first Google login, replacing the email with your own,
-- to make that account the first administrator.
--
